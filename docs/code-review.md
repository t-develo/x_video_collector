...
**該当:** `src/api/XVideoCollector.Functions/Functions/VideoFunctions.cs`

```csharp
_ = Task.Run(() => downloadVideo.ExecuteAsync(video.Id, CancellationToken.None));
```
...
**該当:** 全 Repository 実装（`VideoRepository.cs`, `TagRepository.cs`, `CategoryRepository.cs`, `VideoTagRepository.cs`）
...
**該当:** `infra/modules/sql.bicep`, `infra/main.bicep`
...
**該当:** `infra/modules/functions.bicep`
...
**該当:** `src/frontend/staticwebapp.config.json`
...
**該当:** `src/api/XVideoCollector.Infrastructure/Services/BlobStorageService.cs`
...
**該当:** `src/api/XVideoCollector.Functions/Middleware/ExceptionMiddleware.cs`
...
**該当:** `ListVideosUseCase.cs`, `SearchVideosUseCase.cs`
...
**該当:** `src/api/XVideoCollector.Functions/XVideoCollector.Functions.csproj`
...
**該当:** `Program.cs` と `FunctionHelper.cs`

**問題:** ASP.NET Core の `JsonOptions` と `FunctionHelper.JsonOptions` が独立して定義されている。新しいコンバーター追加時に片方だけ更新する不整合リスクがある。

### 3.5 DurationSeconds が常に 0

**該当:** `src/api/XVideoCollector.Infrastructure/Services/YtDlpDownloadService.cs:180`
...
**該当:** `src/api/XVideoCollector.Application/Dtos/VideoDto.cs`
...
**該当:** `src/api/XVideoCollector.Infrastructure/Services/`
...
**該当:** `VideoRepository`, `TagRepository`, `CategoryRepository`
...
**該当:** `src/frontend/js/pages/videoList.js`

**問題:** クエリパラメータが `tags` だが API 側は `tagIds` を期待する箇所がある。フィルタ状態のURL永続化時に不整合の可能性。
